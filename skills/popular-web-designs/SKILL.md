...
version: 1.1.0
author: Teknium, Claude Code (design systems sourced from VoltAgent/awesome-design-md)
license: MIT
...
4. Pair with desktop_preview to open and inspect the generated HTML directly in the preview pane.

Each template includes a **Hermes Implementation Notes** block at the top with:
- CDN font substitute and Google Fonts `<link>` tag (ready to paste)
- CSS font-family stacks for primary and monospace
- Reminders to use `write_file` for HTML creation and `desktop_preview` + `mcp__computer_use` for verification
...
Write the file with `write_file`, open it with `desktop_preview(action='open', url='file:///absolute/path/to/page.html')`, then verify with `mcp__computer_use(action='capture', mode='vision')` to confirm visual accuracy.
...
- User asks to build a page, UI, or site styled after a known brand or design system
- User says "make it look like [X]" or "in the style of [X]" for any of the 54 catalogued sites
- Generating marketing landing pages, dashboards, or documentation sites that need a polished look
- Pair with `claude-design` when the user wants a thoughtfully designed artifact, not just visual tokens

## Pitfalls

- **Font substitution degrades fidelity** — proprietary fonts (sohne-var, Geist) must be replaced; use the font substitution table and match weight/tracking closely
- **Missing Google Fonts `<link>`** — always paste the CDN link from the template's Hermes notes into the HTML `<head>`
- **CSS custom property drift** — apply all color tokens as `--color-*` variables in `:root`; hardcoding hex values makes maintenance brittle
- **Do not fabricate template content** — always load the actual template file with `skill_view` before generating HTML; never invent token values from memory
- **Responsive behavior** — each template includes breakpoint notes; apply them to avoid desktop-only layouts

## Verification

After generating and writing the HTML file:

1. Open it in the preview pane:
   ```
   desktop_preview(action='open', url='file:///C:/absolute/path/to/page.html')
   ```
2. Capture a visual screenshot to verify:
   ```
   mcp__computer_use(action='capture', mode='vision')
   ```
3. Check against the template's color palette, typography, and spacing — correct any drift before delivering
